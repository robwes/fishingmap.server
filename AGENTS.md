...
- **Regions and regulations**: a `Region` models **who administers a water** (National → ELY → management area), never where the water is or what kind of water it is. The tree is single-parent because jurisdiction is single-parent — a water has one ELY. Geography is not: a water is simultaneously in a latitude band, of a water type, and possibly in a named system, so those belong on the *rule* as conditions, never in the tree. Latitude-band regions were tried and rejected for exactly this reason — see robwes/fishingmap.web#16, which has the worked example (a rule scoped to "Bay of Bothnia north of 63°30′N" is a sea area **and** a latitude, and no single-parent tree holds both). **Never add a region to make a rule's geography work.**
- **Regulation invariants**: scope is XOR — exactly one of `RegionId` or a non-empty `Locations`, enforced by `RegulationsService.ValidateScope`. `GetEffectiveRulesForLocation` resolves the cascade and returns **one winning rule per species**, most specific first, tagged with a `Source` string that is the only inheritance signal the client gets. Protected periods carry **no year** and may wrap past new year — `{12, 1, 1, 31}` means Dec 1 → Jan 31 — so never assume start < end, or every winter closure reads as open.
- **The app covers Uusimaa**: rules recorded on the National region are correct for southern Finland and may not hold nationally (the decree varies by latitude, sea area and water type). Harmless while the app is regional; robwes/fishingmap.web#16 records what changes if it isn't.
